...
app.post('/api/products', requireAdmin, async (req, res) => {
    try {
        const { name, category, price, stock, image_url, image_urls, description } = req.body;
        ...
        const { data, error } = await supabase
            .from('products')
            .insert([{ name, category, price, stock, image_url: cover, image_urls: urls, description: description || '' }])
            .select();
        if (error) throw error;
